...
router.post(
  ...
  [
    ...
    check("email", "Enter a proper email address").isEmail().normalizeEmail(),
...
router.get("/proceed-to-checkout", userVerify, async (req, res) => {
  ...
  res.render("user/proceed-to-checkout", {
...
router.post("/proceed-to-checkout", userVerify, async (req, res) => {
  ...
  getCart
    .placeOrder(req.body, products, totalAmount, CouponName, CouponOffer)
    .then(async (id) => {
      //now we have oder id
...
router.get("/view-product/:id", async (req, res) => {
  ...
  if (ObjectID.isValid(id)) {
    var products = await getOrders.getOrdersproduct(id);
