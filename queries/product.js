...
module.exports = {
    addproduct : (product) => {
        ...
        return new Promise (async (resolve , reject) => {
            try{
                await db.get().collection(collection.product_collection).insertOne(pro).then((data) =>{
    ...
    productdetails : (pdetails) => {
     ...
     return new Promise ((resolve , reject) => {
        db.get().collection(collection.product_collection).findOne({_id:objectID(pdetails)}).
        then((product) => { 
            resolve(product)
        })
     })   
    ...
    deleteProduct : (pid) => {
        return new Promise ( (resolve , reject) => {
            db.get().collection(collection.product_collection).deleteOne({_id:objectID(pid)}).then((response) =>{
                resolve(response)
            })
        })
    },
    addCategory : (category) => {
        return new Promise ((resolve,reject) => {
            db.get().collection(collection.categoy_collection).insertOne(category).then((data) => {
                resolve(data)
            })
        })
    },
    ...
    addDiscountProduct : (id , value) => {
        return new Promise(async (resolve,reject) => {
            await db.get().collection(collection.product_collection).findOneAndUpdate({_id:objectID(id)},
            {
                $set:{
    ...
    getCategories : () => {
        return new Promise(async (resolve,reject) => {
            let getcategory =await  db.get().collection(collection.categoy_collection).find().toArray()
    ...
    categoryDetails : (catId) => {
        return new Promise((resolve,reject) => {
            let category = db.get().collection(collection.categoy_collection).findOne({_id:objectID(catId)})
            resolve(category)
    ...
    editcategory : (catDetails) => {
        ...
        return new Promise ((resolve,reject) => {
            db.get().collection(collection.categoy_collection).updateOne({_id:objectID(catId)},
            {
                $set:{
                    catogory:category,
                    Offer:Offer
    ...
    deletecategory : (catId) => {
        console.log(catId);
        return new Promise ((resolve,reject) => {
            db.get().collection(collection.categoy_collection).deleteOne({_id:objectID(catId)}).then((response) => {
                console.log(response);
                resolve(response)
            })
        })
