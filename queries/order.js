...
module.exports = {
  ...
  getOrdersproduct : (orderid) => {
    ...
    console.log('There is an error occurred in the get order product function' + error)
  }
  },
  getUserOrders : () => {
    return new Promise ( async (resolve,reject) => {
      let userOrders = await db.get().collection(collection.order_collection).find().toArray()
        resolve(userOrders)
    })
  },
  cancelOrder : (details) => {
